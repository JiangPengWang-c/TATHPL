...
def get_args_parser():
    ...
    parser.add_argument('--data_coco', default="/home/featurize/data/MSCOCO", type=str,
                        help='dataset path')
...
###corel5k:
    ...
    # data_path_val = f'{args.data_corel5k}/Corel5k'
    # lable_path_val = f'{args.data_corel5k}/Corel5k/img_to_lable_val.txt'
    ...
    total_loader  = torch.utils.data.DataLoader(
        total_dataset, batch_size=32, shuffle=True,
        num_workers=args.num_workers, pin_memory=True)
    print(len(total_dataset))
    print(total_dataset[4499][1].shape)
    # train_size=int(len(total_dataset)*0.00001)
    # valid_size=len(total_dataset)-train_size
    # train_dataset, val_dataset = random_split(total_dataset, [train_size, valid_size])
    ...
    #model = timm.create_model("vit_base_patch16_224", pretrained=False,num_classes=260)
...
def train_multi_label_coco(model, train_loader, val_loader, lr):
    ...
    #criterion = AsymmetricLoss(gamma_neg=4, gamma_pos=0, clip=0.05, disable_torch_grad_focal_loss=True)nn.BCEWithLogitsLoss()
    ...
    for epoch in range(Epochs):
        ...
        if(patience == 10):
            ...
            print(msg)
            with open('/home/featurize/work/Corel5k_log1.txt','a') as file:
...
def validate_multi(val_loader, model, ema_model):
    ...
    for i, (samples, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            with autocast():
                output_regular = model(samples,target)
                output_regular=Sig(output_regular).cpu()
                output_ema = Sig(ema_model.module(samples,target)).cpu()
    ...
    preds_binary = torch.where(a > 0.5, torch.tensor(1), torch.tensor(0)).cpu()
