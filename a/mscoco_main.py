...
#MS COCO
    ...
    # for name, param in model.named_parameters():
    #     if 'prototype_3' not in name and 'matrix_3' not in name and'prototype_2' not in name and 'matrix_2' not in name and'prototype_1' not in name and 'matrix_1' not in name and'prototype_0' not in name and 'matrix_0' not in name and 'head' not in name:
    #     # if 'head' not in name:
...
#                 output = model(inputData,target_list)  # sigmoid will be done in loss !,sig_loss0,sig_loss1,sig_loss2,sig_loss3,sig_loss0,
#             base_loss,loss= criterion(output, target_list)
#             model.zero_grad()
...
#             ema.update(model)
#             # store information, sig_loss0: {:.2f}, sig_loss1: {:.2f}, sig_loss2: {:.2f}, sig_loss3: {:.2f},sig_loss1.item(),sig_loss2.item(),sig_loss3.item()scheduler.get_last_lr()[0],, Sig_loss0: {:.2f},sig_loss0.item(),sig_loss0.item(), Sig Loss0: {:.2f}
#             if i % 100 == 0:
#                 trainInfoList.append([epoch, i, loss.item()])
#                 print('Epoch [{}/{}], Step [{}/{}], LR {:.1e}, Loss: {:.2f}, Base Loss: {:.2f}'
#                       .format(epoch, Epochs, str(i).zfill(3), str(steps_per_epoch).zfill(3),
#                                scheduler.get_last_lr()[0],\
#                               loss.item(),base_loss.item()))

        model.eval()
     
        mAP_score,preds,targets = validate_multi(val_loader, model, ema)
        model.train()
...
def validate_multi(val_loader, model, ema_model):
    ...
    for i, (img, targetss) in enumerate(val_loader):
        ...
        with torch.no_grad():
            with autocast():
                output_regular = Sig(model(img.cuda(),target_list)).cpu()
                output_ema = Sig(ema_model.module(img.cuda(),target_list)).cpu()
        # for mAP calculation
        ...
        pred = output_regular.data.gt(0.75).long()
