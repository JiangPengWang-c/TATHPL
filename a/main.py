...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='deit_base_topic_patch16_224', type=str, metavar='MODEL',#deit_base_patch16_224
...
#corel5k:
    ...
    total_loader  = torch.utils.data.DataLoader(
        total_dataset, batch_size=16, shuffle=True,
        num_workers=args.num_workers, pin_memory=True)
    print(len(total_dataset))
    # print(total_dataset[4499][1].shape)
...
def train_multi_label_coco(model, train_loader, val_loader, lr):
    ...
    file=open("/home/featurize/data/mAP_base.txt",'w')
   # draw_mAP(mAP)
...
#     scaler = GradScaler()
#     for epoch in range(Epochs):
#         for i, (inputData, targets) in enumerate(train_loader):
...
#                 output = model(inputData,target_list).float()  # sigmoid will be done in loss !
#             #print(output.shape)
#             #print(target.shape)
...
#             highest_mAP = mAP_score
#             # try:
#             #     torch.save(model.state_dict(),'/home/featurize/data/model.pth' )
#             # except:
#             #     pass
...
#         target=targetss[0]
#         topic=[]
#         for j in range(len(targetss[1])):
#             topic.append([float(item) for item in targetss[1][j]])
#         target = target.cuda()
...
#         with torch.no_grad():
#             with autocast():
#                 output_regular = Sig(model(input.cuda(),target_list)).cpu()
#                 output_ema = Sig(ema_model.module(input.cuda(),target_list)).cpu()
#         # for mAP calculation
...
#     micro_f1_res = micro_f1(mcm)
#     macro_f1_res = macro_f1(mcm)
...
#     print(
#             "Hamming_Loss score{:.3f},mAP score regular {:.3f},auc score regular {:.3f},micro_f1_res score{:.3f},macro_f1_res score{:.3f},one_error score {:.3f},  mAP score EMA {:.3f}".format(
#                 Hamming_loss, mAP_score_regular,auc,micro_f1_res, macro_f1_res, one_err, mAP_score_ema))
...
def validate_multi(val_loader, model, ema_model):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        for i in range(len(base_target)):
            base_target[i] = base_target[i].type(torch.float64).to('cuda', non_blocking=True)
        for i in range(len(loss1_target)):

            loss1_target[i] = loss1_target[i].type(torch.float64).to('cuda', non_blocking=True)
        targets_list.append(base_target)
        targets_list.append(loss1_target)
    
        
        #target = target.max(dim=1)[0].type(torch.float64)
        # compute output
        with torch.no_grad():
            with autocast():
                output_regular = model(input.cuda(),targets_list)
                output_regular=Sig(output_regular).cpu()
                #output_ema = Sig(ema_model.module(input.cuda(),targets_list)).cpu()
    ...
    one_err=one_error(torch.cat(targets).numpy(),torch.cat(preds_base))
    auc = get_auc(torch.cat(targets).numpy(),torch.cat(preds_regular).numpy())
    #rank_loss=label_ranking_loss(torch.cat(targets).numpy(),preds_binary)
    #Hamming_loss = computer_hamming_loss(torch.cat(targets_base),torch.cat(preds_base))
    # mAP_score_regular = compute_mAP(torch.cat(targets).numpy(), torch.cat(preds_regular).numpy())
    # mAP_score_ema =compute_mAP(torch.cat(targets).numpy(), torch.cat(preds_ema).numpy())
    mAP_score_regular = mAP(torch.cat(targets).numpy(), torch.cat(preds_regular).numpy(),True)
    #mAP_score_ema = mAP(torch.cat(targets).numpy(), torch.cat(preds_ema).numpy())
    print("Hamming_Loss score{:.3f},micro_f1_res score{:.3f},macro_f1_res score{:.3f},one_error score {:.3f},auc {:.3f}, mAP score regular {:.3f}, mAP score EMA {:.3f}".format(Hamming_loss,micro_f1_res,macro_f1_res,one_err,auc,mAP_score_regular, 0))

    return max(mAP_score_regular,0)
