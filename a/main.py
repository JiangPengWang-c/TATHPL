...
from sklearn.metrics import precision_score, recall_score, f1_score, accuracy_score, zero_one_loss, \
    multilabel_confusion_matrix, label_ranking_loss, roc_auc_score
import timm
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='tit_base_topic_patch16_224', type=str, metavar='MODEL',
...
def train_multi_label_coco(model, train_loader, val_loader, lr):
    ...
    for epoch in range(Epochs):
        criterion = TPLoss(AsymmetricLoss(gamma_neg=4, gamma_pos=0, clip=0.05, disable_torch_grad_focal_loss=True),
                           nn.CrossEntropyLoss(reduction='mean'), epoch)
...
def validate_multi(val_loader, model, ema_model, highest_mAP):
    ...
    print(
        "mAP score regular {:.3f}, mAP score EMA {:.3f},OF1 score{:.3f},OP score{:.3f},OR1 score{:.3f},CF1 score{:.3f},CP score{:.3f},CR score{:.3f},one_error score {:.6f},auc {:.6f}, Hamming_Loss score{:.6f},".format(
            mAP_score_regular, mAP_score_ema, mif1[0], mif1[1], mif1[2], maf1[0], maf1[1], maf1[2], one_err, auc,
            Hamming_loss, ))
