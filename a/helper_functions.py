...
#         #soft:
#         if is_train:
#             file = open("/home/featurize/data/Corel5k/target/train/img_to_tp2(300).txt")
#             total_topics = file.readlines()
#             for i in range(len(total_topics)):
#                 res=total_topics[i].strip().split(':')
#                 res_topic=res[1].split(';')
#                 for j in range(len(res_topic)):
#                     res_topic[j] = res_topic[j].split(' ')
#                 res_topic_float = [list(map(float, tmp)) for tmp in res_topic]
#                 self.topic_dict[res[0]]=res_topic_float             
#         else:
#             file = open("/home/featurize/data/Corel5k/target/val/res50/result50/result50+epoch10.txt")
#             total_topics = file.readlines()
#             for i in range(len(total_topics)):
#                 res=total_topics[i].strip().split(' ')
#                 res_topic=res[1].split(' ')
...
#         target=self.lables[index][2:]
#         topic=self.topic_dict[img_id]
#         target = torch.Tensor(list(map(float,target)))
#         tmp = []
#         tmp.append(target)
#         tmp+=topic
...
#         self.target_transform = target_transform
#         self.img_to_topic={}
#         file = open("/home/featurize/data/targets/img_to_index55(300).txt")
#         total_topics = file.readlines()
#         for index in range(len(total_topics)):
#             topic=total_topics[index].strip().split(':')
#             self.img_to_topic[topic[0]]=topic[1]
#         file.close()
#     def __len__(self):
#         return len(self.lables)
#     def __getitem__(self, index):        
...
#         target=[]
#         if self.is_train:
#             topic=self.img_to_topic[img_id]
#             target.append(topic)#根据是否添加粗类决定要不要注释
#         target = list(map(float, target))#str转int
#         target = torch.Tensor(target)
#         if self.transform is not None:
#             image = self.transform(image)
#         if self.target_transform is not None:
...
#         self.target_transform = target_transform
#         self.cat2cat = dict()
#         self.img_to_topic=dict()
#         for cat in self.coco.cats.keys():
#             self.cat2cat[cat] = len(self.cat2cat)
#         if is_train:
#             file=open("/home/featurize/data/targets2014/train/img_to_index10(300).txt")
#             topics=file.readlines()
#             self.img_to_topic={}
#             for i in range(len(topics)):
#                 res=topics[i].strip().split(':')
...
#                 # self.img_to_topic[res[0]]=res_topic
#         else:
#             file=open("/home/featurize/data/targets2014/val/result10+epoch18(300).txt")
#             topics = file.readlines()
#             for i in range(len(topics)):
#                 res = topics[i].strip().split(':')
#                 res_topic = res[1].split(';')
#                 for j in range(len(res_topic)):
#                     res_topic[j] = res_topic[j].split(',')  # [[0.5,0.5],[0.1,0.5,0.4]]
#                 res_topic_float = [list(map(float, tmp)) for tmp in res_topic]
#                 self.img_to_topic[res[0]] = res_topic_float
...
class CocoDetection_resnet(datasets.coco.CocoDetection):
    def __init__(self, root, annFile, topic_path,transform=None, target_transform=None,is_train=False):
        self.is_train=is_train
        self.root = root
        self.coco = COCO(annFile)
        ...
        self.target_transform = target_transform
        self.img_to_topic={}
        file=open(topic_path)
        topics=file.readlines()
        for i in range(len(topics)):
            res=topics[i].strip().split(':')
            self.img_to_topic[res[0]]=res[1]
        # print(self.cat2cat)
    ...
    def __getitem__(self, index):
        ...
        target=[]
        if self.is_train:
            target.append(self.img_to_topic[str(img_id)])
        target = list(map(float,target))# str转int
        target = torch.Tensor(target)
        path = coco.loadImgs(img_id)[0]['file_name']
        img = Image.open(os.path.join(self.root, path)).convert('RGB')
